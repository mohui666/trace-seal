...
The optional [`guard-event-import.md`](guard-event-import.md) path can attach validated events to an existing run. Issue #36 exposes a compact read-only [`dashboard-data` summary](dashboard-guard-metadata.md) without merging recorder events or changing replay, explain, or Electron UI output.
...
- Read-only dashboard-data metadata only; no Electron UI or installer/release integration.
