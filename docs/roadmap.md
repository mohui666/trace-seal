...
| 桌面实现 | Electron + React + TypeScript + TailwindCSS | 没有 Electron UI 改动；Slint 仅为独立实验 spike |
...
- No v0.3.1 release.
- No Slint default desktop implementation.
...
3. [Slint hello dashboard spike](https://github.com/mohui666/trace-seal/issues/53) and [read-only dashboard-data bridge](https://github.com/mohui666/trace-seal/issues/58)
   - Experimental prototype only after the Slint RFC is accepted.
   - Adds `crates/traceseal-desktop-slint` as a minimal code spike.
   - Keeps static mock dashboard values as the fallback state.
   - Calls only fixed read-only `dashboard-data latest`, `dashboard-data list`, and `dashboard-data policy` commands.
   ...
   - Does not execute target commands, write workspace state, or edit policy.
   - Does not directly load `runs/`, `events.jsonl`, or `manifest.json`.
